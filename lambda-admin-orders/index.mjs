...
/**
 * Get Shopify credentials from DynamoDB or environment
 */
...
async function fetchShopifyOrder(orderId) {
  ...
  if (!SHOPIFY_STORE_DOMAIN || !SHOPIFY_ACCESS_TOKEN) {
    throw new Error('Shopify credentials not configured in DynamoDB or environment');
  }
...
async function healOrder(orderId) {
  ...
  const shopifyCreds = await getShopifyCredentials();
  console.log(`Shopify config - Domain: ${shopifyCreds.SHOPIFY_STORE_DOMAIN ? 'SET' : 'NOT SET'}, Token: ${shopifyCreds.SHOPIFY_ACCESS_TOKEN ? 'SET' : 'NOT SET'}`);
  ...
  try {
    ...
    if (method === 'POST') {
      ...
      if (action === 'updateStatus') {
        const { orderId: updateOrderId, status: newStatus } = body;
        ...
        const validStatuses = ['NEW', 'DISPATCHED', 'PICKED_UP', 'DELIVERED', 'CANCELLED'];
        if (!validStatuses.includes(newStatus)) {
          return {
            statusCode: 400,
            headers,
            body: JSON.stringify({ error: `Invalid status. Valid statuses: ${validStatuses.join(', ')}` }),
          };
        }
        ...
        await docClient.send(new UpdateCommand({
          TableName: TABLE_NAME,
          Key: {
            pk: `ORDER#${updateOrderId}`,
            sk: `SHOPIFY#${updateOrderId}`,
          },
      ...
      if (action === 'updateAllStatus') {
        const { fromStatus, toStatus } = body;
        if (!toStatus) {
          return {
            statusCode: 400,
            headers,
            body: JSON.stringify({ error: 'toStatus is required' }),
          };
        }
        ...
        for (const order of orders) {
          // Skip non-order items
          if (!order.pk?.startsWith('ORDER#')) continue;
          // If fromStatus specified, only update matching orders
          if (fromStatus && order.status !== fromStatus) continue;
      ...
      return {
        statusCode: 400,
        headers,
        body: JSON.stringify({ error: 'Invalid action. Valid actions: heal, cleanupErrors, fixDates, updateStatus' }),
      };
    }
