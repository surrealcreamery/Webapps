    ...
    return (
        <Box>
            ...
            <Box component="form" noValidate sx={{ mt: 2 }}>
                <TextField fullWidth margin="dense" label="First Name" name="firstName" value={contactInfo.firstName}
                    onChange={onFieldChange}
                    error={!!formErrors.firstName}
                    helperText={formErrors.firstName || ' '}
                />
                <TextField fullWidth margin="dense" label="Last Name" name="lastName" value={contactInfo.lastName}
                    onChange={onFieldChange}
                    error={!!formErrors.lastName}
                    helperText={formErrors.lastName || ' '}
                />
                {isHost && (
                    <TextField fullWidth margin="dense" label="Organization Name" name="organizationName" value={contactInfo.organizationName}
                        onChange={onFieldChange}
                        error={!!formErrors.organizationName}
                        helperText={formErrors.organizationName || ' '}
                    />
                )}
                <TextField fullWidth margin="dense" label="Email" name="email" type="email" value={contactInfo.email}
                    onChange={onFieldChange}
