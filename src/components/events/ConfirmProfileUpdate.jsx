...
export const ConfirmProfileUpdate = ({ send, context }) => {
    const machineContext = context.context;
    const { profileMismatch } = machineContext;
    ...
    const diffs = [];
    if (submitted.firstName.trim() !== onFile.firstName.trim()) {
        diffs.push({ field: 'firstName', label: 'First Name', onFile: onFile.firstName, submitted: submitted.firstName });
    }
    if (submitted.lastName.trim() !== onFile.lastName.trim()) {
        diffs.push({ field: 'lastName', label: 'Last Name', onFile: onFile.lastName, submitted: submitted.lastName });
    }
    if (submitted.organizationName &&
        submitted.organizationName.toLowerCase().trim() !== (onFile.organizationName || '').toLowerCase().trim()) {
        diffs.push({ field: 'organizationName', label: 'Organization Name', onFile: onFile.organizationName, submitted: submitted.organizationName });
    }
    ...
    return (
        <Box>
            <Typography variant="h2" gutterBottom>Confirm Your Details</Typography>
