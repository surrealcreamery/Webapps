...
import { ORDERS_API_URL, SHIPDAY_API_URL, authFetch } from '@/constants/admin/adminConstants';
...
const healOrdersApi = async (orderIds) => {
  ...
  const response = await authFetch(ORDERS_API_URL, {
    method: 'POST',
...
const getEstimatesApi = async (shipdayOrderId, locationName) => {
  console.log('[Estimate] Fetching estimates for:', shipdayOrderId, 'location:', locationName);

  const response = await authFetch(SHIPDAY_API_URL, {
    method: 'POST',
    body: JSON.stringify({ action: 'estimate', shipdayOrderId, locationName }),
  });
...
const dispatchOrderApi = async (shipdayOrderId, carrierId, locationName) => {
  console.log('[Dispatch] Assigning carrier:', carrierId, 'to order:', shipdayOrderId, 'location:', locationName);

  const response = await authFetch(SHIPDAY_API_URL, {
    method: 'POST',
    body: JSON.stringify({ action: 'dispatch', shipdayOrderId, carrierId, locationName }),
  });
...
const updateOrderStatus = async (orderId, status) => {
  console.log('[Status] Updating order:', orderId, 'to status:', status);
...
export default function InStoreOrders() {
  ...
  const { data: orders = [], isLoading, isError, refetch } = useQuery({
    queryKey: ['admin', 'in-store-orders', selectedDate],
  ...
  const dispatchMutation = useMutation({
    mutationFn: async ({ shipdayOrderId, carrierId, locationName, orderId }) => {
      // First dispatch via Shipday
      const result = await dispatchOrderApi(shipdayOrderId, carrierId, locationName);
  ...
  const handleDispatchClick = useCallback(async (order) => {
    ...
    try {
      const locationName = order.locationName || order.location || '';
      const result = await getEstimatesApi(order.shipdayOrderId, locationName);
  ...
  const handleSelectCarrier = useCallback((carrierId) => {
    if (dispatchOrder?.shipdayOrderId && carrierId) {
      dispatchMutation.mutate({
        ...
        carrierId,
        locationName: dispatchOrder.locationName || dispatchOrder.location || '',
